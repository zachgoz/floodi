...
async function requestWithCache(url: string, ttl = 30000): Promise<any> {
  if (requestCache[url]) {
    markPerf('network.requestCache.hit', getRequestMetadata(url));
    return requestCache[url];
  }

  const fetchPromise = (async () => {
    const metadata = getRequestMetadata(url);
    const res = await measurePerf('network.fetch', () => fetch(url, { mode: 'cors', cache: 'default' }), metadata);
...
function getRequestMetadata(url: string) {
  try {
    const parsed = new URL(url, window.location.origin);
    return {
      host: parsed.host || 'same-origin',
      path: parsed.pathname,
      product: parsed.searchParams.get('product') ?? undefined,
      platform: parsed.searchParams.get('platform') ?? undefined,
      beginDate: parsed.searchParams.get('begin_date') ?? undefined,
      endDate: parsed.searchParams.get('end_date') ?? undefined,
    };
  } catch {
    return { url };
  }
...
export async function fetchObservedWaterLevels(opts: {
  ...
  if (isNoaaRequested) {
    ...
    const results = await measurePerf('noaa.fetchObservedWaterLevels.noaaChunks', () => Promise.all(chunks.map(c => fetchChunk(c.s, c.e))), {
      ...metadata,
  ...
  if (isFimanRequested && days <= 90) {
    ...
    const platformResults = await measurePerf('noaa.fetchObservedWaterLevels.fimanPlatforms', () => Promise.all(sensors.map(fetchOnePlatform)), {
      ...metadata,
      platforms: sensors,
    });
...
export async function fetchPredictions(opts: {
  ...
  const results = await measurePerf('noaa.fetchPredictions.chunks', () => Promise.all(chunks.map(c => fetchChunk(c.s, c.e))), {
    ...metadata,
    chunks: chunks.length,
  });
...
export async function buildAdjustedFuture(opts: {
  ...
  const metadata = {
    ...
    lookbackHours,
    lookaheadHours,
    hasExistingObserved: !!existingObserved,
    hasExistingPredicted: !!existingPredicted,
  };

  return measurePerf('noaa.buildAdjustedFuture.total', async () => {
